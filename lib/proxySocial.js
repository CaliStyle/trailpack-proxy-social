...
module.exports = {
     ...
     */
  init: (app) => {
    // const proxysocial = app.services.ProxySocialService.proxysocial
  },
   ...
   */
  addRoutes: (app) => {
    const prefix = _.get(app.config, 'proxysocial.prefix') || _.get(app.config, 'footprints.prefix')
   ...
   */
  copyDefaults: (app) => {
    app.config.proxySocialDefaults = _.clone(app.config.proxySocial)
    return Promise.resolve({})
  }
}
